...
class ModelArguments:
    ...
    dtype: Optional[str] = field(
        ...
        metadata={
            "help":
            "Floating-point format in which the computations will be performed (not the model weights). Choose one of `[float32, float16, bfloat16]`."
        },
    )
    restore_state: Optional[bool] = field(
        ...
        metadata={
            "help":
            "Restore optimizer and training state. Can be True (will retrieve associated wandb artifact), a local directory or a Google bucket path."
        },
    )
    ...
    def get_opt_state(self):
        with tempfile.TemporaryDirectory(
        ) as tmp_dir:  # avoid multiple artifact copies
            if self.restore_state is True:
                ...
                if artifact.metadata.get("bucket_path"):
                    ...
                    self.restore_state = str(
                        Path(artifact_dir) / "opt_state.msgpack")

            if self.restore_state.startswith("gs://"):
                bucket_path = Path(
                    self.restore_state[5:]) / "opt_state.msgpack"
...
class DataTrainingArguments:
    ...
    encoding_column: Optional[str] = field(
        ...
        metadata={
            "help":
            "The name of the column in the datasets containing the image encodings."
        },
    )
    ...
    train_file: Optional[str] = field(
        ...
        metadata={
            "help":
            "The input training data file (glob & braceexpand acceptable)."
        },
    )
    validation_file: Optional[str] = field(
        ...
        metadata={
            "help":
            "An optional input evaluation data file (glob & braceexpand acceptable)."
        },
    )
    ...
    use_auth_token: Optional[bool] = field(
        ...
        metadata={
            "help":
            "Whether to use the authentication token for private datasets."
        },
    )
    shard_by_host: Optional[bool] = field(
        ...
        metadata={
            "help":
            "Whether to shard data files by host in multi-host environments."
        },
    )
    blank_caption_prob: Optional[float] = field(
        ...
        metadata={
            "help":
            "Probability of removing some captions for classifier-free guidance."
        },
    )
    ...
    multi_eval_ds: Optional[bool] = field(
        ...
        metadata={
            "help":
            "Whether to look for multiple validation datasets (local support only)."
        },
    )
    ...
    preprocessing_num_workers: Optional[int] = field(
        ...
        metadata={
            "help":
            "The number of processes to use for the preprocessing. Not used in streaming mode."
        },
    )
    overwrite_cache: bool = field(
        ...
        metadata={
            "help":
            "Overwrite the cached training and evaluation sets. Not used in streaming mode."
        },
    )
    ...
    seed_dataset: int = field(
        ...
        metadata={
            "help":
            "Random seed for the dataset that will be set at the beginning of training."
        },
    )
...
class TrainingArguments:
    ...
    per_device_eval_batch_size: Optional[int] = field(
        ...
        metadata={
            "help":
            "Batch size per data parallel device for evaluation. Same as training batch size if not set."
        },
    )
    ...
    gradient_accumulation_steps: int = field(
        ...
        metadata={
            "help":
            "Number of updates steps to accumulate before performing an update pass."
        },
    )
    ...
    optim: str = field(
        ...
        metadata={
            "help":
            'The optimizer to use. Can be "distributed_shampoo" (default), "adam" or "adafactor"'
        },
    )
    ...
    block_size: int = field(
        default=1024,
        metadata={
            "help": "Chunked size for large layers with Distributed Shampoo."
        },
    )
    preconditioning_compute_steps: int = field(
        default=10,
        metadata={"help": "Number of steps to update preconditioner."})
    skip_preconditioning_dim_size_gt: int = field(
        default=4096,
        metadata={
            "help": "Max size for preconditioning with Distributed Shampoo."
        },
    )
    graft_type: str = field(
        ...
        metadata={
            "help":
            "The type of grafting to use. Can be 'rmsprop_normalized' (default), 'rmsprop', 'adagrad', 'adagrad_normalized', 'sgd' or 'sqrt_n'"
        },
    )
    nesterov: bool = field(
        default=False,
    ...
    shard_shampoo_across: str = field(
        ...
        metadata={
            "help":
            "Whether to shard the optimizer across data devices (dp), model devices (mp) or both (2d)."
        },
    )
    ...
    lr_transition_steps: int = field(
        ...
        metadata={
            "help":
            "Number of transition steps associated with learning rate decay when using exponential decay."
        },
    )
    ...
    logging_steps: int = field(default=40,
    ...
    log_histogram_steps: int = field(
        ...
        metadata={
            "help":
            "Log parameters and gradients histograms at this frequency. Slows down training."
        },
    )
    ...
    seed_model: int = field(
        ...
        metadata={
            "help":
            "Random seed for the model that will be set at the beginning of training."
        },
    )
    ...
    mp_devices: Optional[int] = field(
        ...
        metadata={
            "help":
            "Number of devices required for model parallelism. The other dimension of available devices is used for data parallelism."
        },
    )
    ...
    def __post_init__(self):
        ...
        if (os.path.exists(self.output_dir) and os.listdir(self.output_dir)
                and self.do_train and not self.overwrite_output_dir):
            raise ValueError(
        ...
        assert (self.mp_devices >
                0), f"Number of devices for model parallelism must be > 0"
        assert (
...
def main():
    ...
    # Load tokenizer
    tokenizer = GameCoordinateTokenizer()
    ...
    batch_size_per_node = (batch_size_per_node_per_grad_step *
                           training_args.gradient_accumulation_steps)
    batch_size_per_step = batch_size_per_node * jax.process_count()
    eval_batch_size_per_node = (training_args.per_device_eval_batch_size *
                                jax.local_device_count() //
                                training_args.mp_devices)
    eval_batch_size_per_step = eval_batch_size_per_node * jax.process_count()
    len_train_dataset, len_eval_dataset = dataset.length
    steps_per_epoch = (len_train_dataset // batch_size_per_node
                       if len_train_dataset is not None else None)
    num_train_steps = (steps_per_epoch *
                       num_epochs if steps_per_epoch is not None else None)
    ...
    # create optimizer
    trainable_params_shape = trainable_params(params_shape,
                                              training_args.embeddings_only)
    if training_args.optim == "distributed_shampoo":
        ...
        statistics_partition_spec = (PartitionSpec(
            None, training_args.shard_shampoo_across,
            None) if training_args.shard_shampoo_across != "2d" else
                                     PartitionSpec(None, "dp", "mp"))
        opt = distributed_shampoo(
            ...
            preconditioning_compute_steps=training_args.
            ...
            if training_args.shard_shampoo_across != "2d" else PartitionSpec(
                "mp" if training_args.mp_devices > training_args.dp_devices
                else "dp",
                None,
                None,
            ),
            ...
            skip_preconditioning_dim_size_gt=training_args.
        ...
        for k, p in split_params(trainable_params_shape).items():
            ...
            opt_fn[k] = NamedTuple("opt_fn",
                                   pspec_fn=Any,
                                   shape_and_dtype_fn=Any)(
                                       optimizer[k].pspec_fn,
                                       optimizer[k].shape_and_dtype_fn)
            optimizer[k] = optax.GradientTransformation(
                optimizer[k].init_fn, update_fn)
        ...
        optimizer = {
            k: optimizer
            for k in split_params(trainable_params_shape)
        }
    ...
    def get_opt_state_spec_and_shape():
        ...
        if training_args.optim == "adafactor":
            ...
            split_spec = split_params(
                set_partitions(trainable_params_shape, False))
            opt_state_spec = {}
            for k, p in split_params(trainable_params_shape).items():
                if "scanned" in k:
                    p = jax.eval_shape(
                ...
                if training_args.optim == "adam":
                    opt_state_spec[k] = jax.tree_util.tree_map(
                        ...
                        is_leaf=lambda x: isinstance(x, (FrozenDict, optax.
                                                         EmptyState)),
                    )
    ...
    class TrainState(struct.PyTreeNode):
        ...
        def apply_gradients(self, *, grads, **kwargs):
            grads = split_params(
                trainable_params(grads, training_args.embeddings_only))
            params = split_params(
            ...
            for k, param in params.items():
                ...
                if "scanned" in k:
                    update_fn = jax.vmap(update_fn,
                                         in_axes=(0, 0, 0),
                                         out_axes=(0, 0))
                updates, new_opt_state = update_fn(grads[k], self.opt_state[k],
                                                   param)
            ...
            params, new_params = traverse_util.flatten_dict(
                unfreeze(self.params)), traverse_util.flatten_dict(
                    unfreeze(params))
        ...
        def create(cls, *, apply_fn, params, tx, **kwargs):
            ...
            for k, p in split_params(
                    trainable_params(params,
                                     training_args.embeddings_only)).items():
                init_fn = tx[k].init
                if "scanned" in k:
    ...
    with mesh:
        ...
        if not model_args.restore_state:
            ...
            state = pjit(
                ...
                if model_args.model_name_or_path else None,
    ...
    if use_vmap_trick:
        grad_param_spec = jax.tree_util.tree_map(
            lambda x: PartitionSpec(*("dp", ) + (x if x is not None else
                                                 (None, ))),
            param_spec,
        )
    ...
    def train_step(state, batch, train_time):
        ...
        def compute_loss(params, minibatch, dropout_rng):
            ...
            logits = state.apply_fn(**minibatch,
                                    params=params,
                                    dropout_rng=dropout_rng,
                                    train=True)[0]
        ...
        def loss_and_grad(grad_idx, dropout_rng):
            ...
            if use_vmap_trick:
                ...
                loss, grads = jax.vmap(grad_fn,
                                       in_axes=(None, 0, None),
                                       out_axes=(0, 0))(state.params,
                                                        minibatch, dropout_rng)
        ...
        if training_args.log_norm_steps:
            zeros_norm = jax.tree_util.tree_map(lambda _: jnp.float32(0),
                                                params)

            def norm(val):
                return jax.tree_util.tree_map(lambda x: jnp.linalg.norm(x),
                                              val)

            gradients_norm = maybe_fn(norm, grads, zeros_norm,
                                      training_args.log_norm_steps)
            params_norm = maybe_fn(norm, params, zeros_norm,
                                   training_args.log_norm_steps)
        ...
        if training_args.log_histogram_steps:
            ...
            gradients_hist = maybe_fn(histogram, grads, zeros_hist,
                                      training_args.log_histogram_steps)
            params_hist = maybe_fn(histogram, params, zeros_hist,
                                   training_args.log_histogram_steps)
    ...
    # Define eval fn
    eval_model = (model if model_args.dtype == "float32" else DalleBart(
        model.config,
        seed=training_args.seed_model,
        dtype=jnp.float32,
        _do_init=False,
    ...
    p_train_step = pjit(
        ...
        in_axis_resources=(
            state_spec,
            grad_batch_spec
            if training_args.gradient_accumulation_steps > 1 else batch_spec,
            None,
        ),
    ...
    def run_evaluation():
        ...
        if training_args.do_eval:
            ...
            val_datasets = list(dataset.other_eval_datasets.keys(
            ) if hasattr(dataset, "other_eval_datasets") else [])
            val_datasets += ["eval"]
            for val_dataset in val_datasets:
                ...
                eval_steps = (len_eval_dataset // eval_batch_size_per_step
                              if len_eval_dataset is not None else None)
                eval_loss = []
                for batch in tqdm(
                        eval_loader,
                        desc="Evaluating...",
                    ...
                    batch = jax.tree_util.tree_map(
                        lambda x: x.reshape((jax.process_count(
                        ), eval_batch_size_per_node) + x.shape[1:]),
                        batch,
                    )
                    batch = jax.tree_util.tree_map(
    ...
    def run_save_model(state, eval_metrics=None):
        if jax.process_index() == 0:
            ...
            if use_bucket:
                bucket_path = Path(
                    output_dir[5:]) / wandb.run.id / f"step_{state.step}"
            ...
            metrics_logger.log_time("save_model",
                                    time.perf_counter() - start_save_time)
        ...
        for epoch in epochs:
            ...
            if training_args.do_train:
                ...
                for batch in tqdm(
                    ...
                    bs_shape = (
                        (batch_size_per_node_per_grad_step *
                         node_groups, ) if not use_vmap_trick else (
                             jax.local_device_count() * node_groups //
                    ...
                    if training_args.gradient_accumulation_steps > 1:
                        ...
                        bs_shape = (training_args.gradient_accumulation_steps,
                                    ) + bs_shape
